...
### ✨ New Features
- **iTrip Next Guest Date**: Capture and utilize "Next Booking" field from iTrip CSV exports
  - Automatically extracts next guest arrival date from iTrip CSV data
  - Stores in new "iTrip Next Guest Date" field in Airtable
  - `find-next-guest-date.js` automation now prioritizes iTrip's authoritative data
  - Eliminates need for complex date calculations when iTrip data is available
  - Supports multiple column name variations: "Next Booking", "next booking", "Next booking"
  - Parses datetime format "YYYY-MM-DD HH:MM:SS" to ISO date format
  - Falls back to existing calculation logic for non-iTrip reservations

### 🔧 Improvements
- **Enhanced Error Messages in Automation Status**:
  - Replaced generic "Unknown error - check logs" with specific, actionable error messages
  - Added comprehensive error pattern detection in `run_automation.py`
  - Now reports specific errors like "Airtable field 'X' does not exist" or "3 reservations have properties not found in Airtable"
  - Improved error extraction for CSV processing, ICS processing, and Python exceptions
  - Better handling of Airtable API errors, network issues, and permission problems

### 📝 Documentation
- Added comprehensive documentation for iTrip Next Guest Date feature
- Updated feature documentation in docs_v2/docs/features/
