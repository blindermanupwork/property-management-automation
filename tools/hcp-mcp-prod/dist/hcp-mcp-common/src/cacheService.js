...
import { CACHE_DEFAULTS } from './index.js';
export class CacheService {
     ...
     */
    async cacheResponse(data, operation, environment, queryParams) {
        try {
     ...
     */
    async listCacheFiles(environment, operation) {
        try {
    ...
    async searchCache(filePath, searchTerm, fieldPath) {
        try {
            ...
            let searchData = Array.isArray(data) ? data : data.data || [];
    ...
    generateHash(data, operation, queryParams) {
        const content = JSON.stringify({ data, operation, queryParams });
        return crypto.createHash('md5').update(content).digest('hex').substring(0, 8);
    }
    getSubDirectory(operation) {
        if (operation.includes('customer'))
            return 'customers';
        if (operation.includes('employee'))
            return 'employees';
        if (operation.includes('job_type'))
            return 'job_types';
        if (operation.includes('appointment'))
            return 'appointments';
        if (operation.includes('line_item'))
            return 'line_items';
        if (operation.includes('job'))
            return 'jobs';
        return 'misc';
    }
    getRecordCount(data) {
    ...
    async getAllSubDirectories(baseDir) {
        try {
