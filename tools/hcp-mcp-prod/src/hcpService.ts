...
export class HCPService {
  ...
  async getJobsByAddress(addressId: string, params: {
    ...
    try {
      ...
      if (!customerId && customers.length > 0) {
        for (const customer of customers) {
          if (customer.addresses?.some((addr: any) => addr.id === addressId)) {
      ...
      if (!customerId && !('_cached' in customersResponse)) {
        ...
        while (hasMore && !customerId) {
          ...
          if ('_cached' in moreCustomers) {
            ...
            const customerList = (moreCustomers as any).customers || [];
      ...
      while (hasMore) {
        ...
        if ('_cached' in jobsResponse) {
          ...
          const jobsList = (jobsResponse as any).jobs || [];
          // Filter by address.id (customer may have multiple addresses)
          jobs = jobsList.filter((job: any) => job.address?.id === addressId);
  ...
  private slimCustomer(customer: any): any {
    return {
      id: customer.id,
      first_name: customer.first_name,
      last_name: customer.last_name,
