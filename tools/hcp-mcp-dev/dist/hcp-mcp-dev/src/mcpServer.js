...
import { validateString, validateEmail, validatePhoneNumber, validateDate, validateHCPId, validateJobStatus, validateEmployeeRole, validateAppointmentStatus, validateSortDirection, validateCustomerSortField, validatePaginationParams, validateSchedule, validateLineItem, validateArray, MCP_TOOL_NAMES } from '../../hcp-mcp-common/src/index.js';
export class HCPMCPServer {
    server;
    hcpService;
    environment;
    constructor(hcpService, environment) {
    ...
    async handleListCustomers(args) {
        ...
        if (args.q)
            params.q = validateString(args.q, 'q');
        if (args.sort_by)
            params.sort_by = validateCustomerSortField(args.sort_by);
        if (args.sort_direction)
            params.sort_direction = validateSortDirection(args.sort_direction);
        if (args.created_after)
            params.created_after = validateDate(args.created_after, 'created_after');
        if (args.created_before)
            params.created_before = validateDate(args.created_before, 'created_before');
        if (args.tags)
            params.tags = validateArray(args.tags, 'tags');
    ...
    async handleCreateCustomer(args) {
        const data = {};
        if (args.first_name)
            data.first_name = validateString(args.first_name, 'first_name');
        if (args.last_name)
            data.last_name = validateString(args.last_name, 'last_name');
        if (args.company)
            data.company = validateString(args.company, 'company');
        if (args.email)
            data.email = validateEmail(args.email);
        if (args.mobile_number)
            data.mobile_number = validatePhoneNumber(args.mobile_number, 'mobile_number');
        if (args.home_number)
            data.home_number = validatePhoneNumber(args.home_number, 'home_number');
        if (args.work_number)
            data.work_number = validatePhoneNumber(args.work_number, 'work_number');
        if (args.lead_source)
            data.lead_source = validateString(args.lead_source, 'lead_source');
        if (args.notes)
            data.notes = validateString(args.notes, 'notes');
        if (args.tags)
            data.tags = validateArray(args.tags, 'tags');
        if (args.addresses)
    ...
    async handleGetCustomerJobs(args) {
        const customerId = validateHCPId(args.customer_id, 'customer');
    ...
    async handleListEmployees(args) {
        ...
        if (args.is_active !== undefined)
            params.is_active = Boolean(args.is_active);
        if (args.role)
            params.role = validateEmployeeRole(args.role);
    ...
    async handleCreateEmployee(args) {
        const data = {
        ...
        if (args.mobile_number)
            data.mobile_number = validatePhoneNumber(args.mobile_number, 'mobile_number');
        if (args.color_hex)
            data.color_hex = validateString(args.color_hex, 'color_hex');
        if (args.tags)
            data.tags = validateArray(args.tags, 'tags');
    ...
    async handleUpdateEmployee(args) {
        const employeeId = validateHCPId(args.employee_id, 'employee');
        const data = {};
        if (args.first_name)
            data.first_name = validateString(args.first_name, 'first_name');
        ...
        if (args.mobile_number)
            data.mobile_number = validatePhoneNumber(args.mobile_number, 'mobile_number');
        if (args.role)
            data.role = validateEmployeeRole(args.role);
        if (args.is_active !== undefined)
            data.is_active = Boolean(args.is_active);
        if (args.color_hex)
            data.color_hex = validateString(args.color_hex, 'color_hex');
        if (args.tags)
            data.tags = validateArray(args.tags, 'tags');
    ...
    async handleGetEmployeeSchedule(args) {
        const employeeId = validateHCPId(args.employee_id, 'employee');
    ...
    async handleListJobs(args) {
        ...
        const params = { ...paginationParams };
        if (args.customer_id)
            params.customer_id = validateHCPId(args.customer_id, 'customer');
        if (args.work_status)
            params.work_status = validateJobStatus(args.work_status);
        if (args.scheduled_start_min)
            params.scheduled_start_min = validateDate(args.scheduled_start_min, 'scheduled_start_min');
        if (args.scheduled_start_max)
            params.scheduled_start_max = validateDate(args.scheduled_start_max, 'scheduled_start_max');
        if (args.assigned_employee_id)
            params.assigned_employee_id = validateHCPId(args.assigned_employee_id, 'employee');
        if (args.job_type_id)
            params.job_type_id = validateHCPId(args.job_type_id, 'job_type');
    ...
    async handleCreateJob(args) {
        ...
        if (args.assigned_employee_ids) {
            data.assigned_employee_ids = validateArray(args.assigned_employee_ids, 'assigned_employee_ids', (id) => validateHCPId(id, 'employee'));
        }
        if (args.line_items) {
            data.line_items = validateArray(args.line_items, 'line_items', validateLineItem);
        }
        if (args.job_type_id)
            data.job_type_id = validateHCPId(args.job_type_id, 'job_type');
        if (args.notes)
            data.notes = validateString(args.notes, 'notes');
        if (args.description)
            data.description = validateString(args.description, 'description');
        if (args.work_status)
            data.work_status = validateJobStatus(args.work_status);
    ...
    async handleUpdateJob(args) {
        const jobId = validateHCPId(args.job_id, 'job');
        const data = {};
        if (args.customer_id)
            data.customer_id = validateHCPId(args.customer_id, 'customer');
        if (args.address_id)
            data.address_id = validateHCPId(args.address_id, 'address');
        if (args.schedule)
            data.schedule = validateSchedule(args.schedule);
        if (args.work_status)
            data.work_status = validateJobStatus(args.work_status);
        if (args.assigned_employee_ids) {
            data.assigned_employee_ids = validateArray(args.assigned_employee_ids, 'assigned_employee_ids', (id) => validateHCPId(id, 'employee'));
    ...
    async handleRescheduleJob(args) {
        const jobId = validateHCPId(args.job_id, 'job');
    ...
    async handleGetJobLineItems(args) {
        const jobId = validateHCPId(args.job_id, 'job');
    ...
    async handleUpdateJobLineItems(args) {
        const jobId = validateHCPId(args.job_id, 'job');
    ...
    async handleCreateJobLineItem(args) {
        const jobId = validateHCPId(args.job_id, 'job');
    ...
    async handleGetJobLineItem(args) {
        const jobId = validateHCPId(args.job_id, 'job');
    ...
    async handleUpdateJobLineItem(args) {
        const jobId = validateHCPId(args.job_id, 'job');
        const lineItemId = validateHCPId(args.line_item_id, 'line_item');
        const lineItem = validateLineItem(args);
    ...
    async handleDeleteJobLineItem(args) {
        const jobId = validateHCPId(args.job_id, 'job');
        const lineItemId = validateHCPId(args.line_item_id, 'line_item');
    ...
    async handleCreateJobType(args) {
        const data = {
    ...
    async handleUpdateJobType(args) {
        const jobTypeId = validateHCPId(args.job_type_id, 'job_type');
        const data = {};
        if (args.name)
            data.name = validateString(args.name, 'name');
        if (args.description)
            data.description = validateString(args.description, 'description');
        if (args.color_hex)
            data.color_hex = validateString(args.color_hex, 'color_hex');
    ...
    async handleListAppointments(args) {
        const params = {};
        if (args.job_id)
            params.job_id = validateHCPId(args.job_id, 'job');
        if (args.scheduled_start_min)
            params.scheduled_start_min = validateDate(args.scheduled_start_min, 'scheduled_start_min');
        if (args.scheduled_start_max)
            params.scheduled_start_max = validateDate(args.scheduled_start_max, 'scheduled_start_max');
        if (args.assigned_employee_id)
            params.assigned_employee_id = validateHCPId(args.assigned_employee_id, 'employee');
        if (args.status)
            params.status = validateAppointmentStatus(args.status);
    ...
    async handleGetAppointment(args) {
    ...
    async handleCreateAppointment(args) {
        const data = {
            ...
            assigned_employee_ids: validateArray(args.assigned_employee_ids, 'assigned_employee_ids', (id) => validateHCPId(id, 'employee'))
        };
        if (args.notes)
            data.notes = validateString(args.notes, 'notes');
    ...
    async handleUpdateAppointment(args) {
        const appointmentId = validateHCPId(args.appointment_id, 'appointment');
        const data = {};
        if (args.job_id)
            data.job_id = validateHCPId(args.job_id, 'job');
        if (args.scheduled_start)
            data.scheduled_start = validateDate(args.scheduled_start, 'scheduled_start');
        if (args.scheduled_end)
            data.scheduled_end = validateDate(args.scheduled_end, 'scheduled_end');
        if (args.assigned_employee_ids) {
            data.assigned_employee_ids = validateArray(args.assigned_employee_ids, 'assigned_employee_ids', (id) => validateHCPId(id, 'employee'));
        }
        if (args.status)
            data.status = validateAppointmentStatus(args.status);
        if (args.notes)
            data.notes = validateString(args.notes, 'notes');
    ...
    async handleDeleteAppointment(args) {
        const appointmentId = validateHCPId(args.appointment_id, 'appointment');
    ...
    async handleSearchAddresses(args) {
        const params = {};
        if (args.street)
            params.street = validateString(args.street, 'street');
        if (args.city)
            params.city = validateString(args.city, 'city');
        if (args.state)
            params.state = validateString(args.state, 'state');
        if (args.zip)
            params.zip = validateString(args.zip, 'zip');
        if (args.customer_name)
            params.customer_name = validateString(args.customer_name, 'customer_name');
        if (args.customer_id)
            params.customer_id = validateHCPId(args.customer_id, 'customer');
    ...
    async handleGetJobsByAddress(args) {
    ...
    async handleGetCacheSummary(args) {
    ...
    async handleAnalyzeLaundryJobs(args) {
    ...
    async handleAnalyzeServiceItems(args) {
    ...
    async handleAnalyzeCustomerRevenue(args) {
    ...
    async handleAnalyzeJobStatistics(args) {
    ...
    async handleAnalyzeTowelUsage(args) {
