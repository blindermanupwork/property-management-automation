...
0 6 * * * $AUTOMATION_DIR/run_prod.sh >> $AUTOMATION_DIR/logs/cron.log 2>&1

# Health monitoring every 30 minutes
*/30 * * * * $AUTOMATION_DIR/monitor.sh

# Backup automation weekly on Sundays at 2 AM  
0 2 * * 0 $AUTOMATION_DIR/backup.sh >> $AUTOMATION_DIR/logs/backup.log 2>&1
