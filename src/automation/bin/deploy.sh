...
run_tests() {
    ...
    source "$AUTOMATION_DIR/config/environments/dev/.env"
    
    # Run a quick test of the automation system
    cd "$AUTOMATION_DIR"
    timeout 300 python3 run_anywhere.py --test 2>/dev/null || {
...
deploy_to_production() {
    ...
    source "$AUTOMATION_DIR/config/environments/prod/.env"
