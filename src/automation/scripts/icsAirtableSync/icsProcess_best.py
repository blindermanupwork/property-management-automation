...
from ultra_debug_timing import ultra_timing_debug, log_critical_timing
    ...
    def flush(self):
        ...
        try:
            if self.op == "update":
                ...
                # create
                # CRITICAL DEBUGGING: Log exact time when creating records
                import datetime as dt
                logging.info(f"🔍 CRITICAL: BatchCollector creating {len(self.records)} records at {dt.datetime.now().strftime('%H:%M:%S.%f')[:12]}")
                # Also log what's being created
                for i, r in enumerate(self.records):
                    uid = r.get("fields", {}).get("Reservation UID", "unknown")
                    prop = r.get("fields", {}).get("Property ID", ["unknown"])[0] if r.get("fields", {}).get("Property ID") else "unknown"
                    logging.info(f"  Creating record {i+1}: UID={uid}, Property={prop}")
...
# ---------------------------------------------------------------------------
# InMemoryCollector - Collects all operations before execution
# ---------------------------------------------------------------------------
class InMemoryCollector:
    """Collects all operations in memory before execution to prevent creating-then-marking-old."""
    ...
    def __init__(self):
        ...
        # LODGIFY FIX: Track property+dates+type combinations processed
        # This prevents removal when Lodgify changes UIDs
        self.processed_property_dates = set()
        
    def add_create(self, uid, feed_url, data):
        """Add a create operation with duplicate detection."""
        # Extract key fields for duplicate detection
        property_id = data['Property ID'][0] if data.get('Property ID') else None
        checkin = data['Check-in Date']
        checkout = data['Check-out Date']
        entry_type = data['Entry Type']
        
        # Check if we already have a pending create for the same property/dates/type
        # This prevents Lodgify's dynamic UIDs from creating duplicates
        for pending in self.to_create:
            if (pending['property_id'] == property_id and 
                pending['checkin'] == checkin and 
                pending['checkout'] == checkout and 
                pending['entry_type'] == entry_type):
                # Duplicate detected in batch - skip this one
                print(f"⚠️  BATCH DUPLICATE DETECTED: Skipping {uid} for property {property_id}, {checkin} to {checkout}")
                self.processed_uids.add((uid, feed_url))
                return
        ...
        # LODGIFY FIX: Track property+dates+type for this record
        if property_id:
            self.processed_property_dates.add((property_id, checkin, checkout, entry_type))
            logging.debug(f"📍 Tracking property+dates: {property_id}, {checkin} to {checkout}, {entry_type}")
    ...
    def add_remove(self, record):
        ...
        self.to_remove.append({
            ...
            'property_id': fields.get('Property ID', [None])[0] if fields.get('Property ID') else None,
    ...
    def cross_reference_operations(self):
        ...
        for create in self.to_create:
            ...
            if matching_removals:
                ...
                logging.info(f"🔄 UID CHANGE DETECTED: {prop_name}")
                logging.info(f"   Dates: {create['checkin']} to {create['checkout']}")
                logging.info(f"   Old UID: {matching_removals[0]['uid']}")
                logging.info(f"   New UID: {create['uid']}")
            else:
        ...
        logging.info(f"   Original creates: {len(self.to_create)}")
        logging.info(f"   Original removes: {len(self.to_remove)}")
        logging.info(f"   UID changes detected: {len(uid_changes_detected)}")
        logging.info(f"   Filtered creates: {len(filtered_creates)}")
        logging.info(f"   Filtered removes: {len(filtered_removes_final)}")
...
async def fetch_ics_async(session, url):
    """Asynchronously fetches ICS content from a URL with 3x retry logic."""
    max_retries = 3
    retry_delay = 2  # seconds between retries
    
    for attempt in range(1, max_retries + 1):
        try:
            logging.info(f"Fetching ICS from {url} (attempt {attempt}/{max_retries})")
            async with session.get(url, timeout=15) as resp:
                if resp.status != 200:
                    error_msg = f"HTTP {resp.status} - {resp.reason}"
                    if attempt < max_retries:
                        logging.warning(f"Attempt {attempt} failed for {url}: {error_msg}. Retrying in {retry_delay}s...")
                        await asyncio.sleep(retry_delay)
                        continue
                    else:
                        logging.error(f"All {max_retries} attempts failed for {url}: {error_msg}")
                        return url, False, None, f"Failed after {max_retries} attempts: {error_msg}"
                ...
                logging.info(f"Successfully fetched ICS from {url} on attempt {attempt}")
    ...
    return url, False, None, f"Failed after {max_retries} attempts"
...
def check_for_duplicate_with_tracking(table, property_id, checkin_date, checkout_date, entry_type, create_batch=None):
    ...
    try:
        # Get ALL records for this property (including Removed/Old to handle Lodgify UID changes)
        # IMPORTANT: We must check ALL statuses because Lodgify changes UIDs frequently
        # and we need to find previously removed records to update them instead of creating new ones
        ...
        # ENHANCED: Also check InMemoryCollector if provided
        if create_batch and hasattr(create_batch, 'to_create'):
            pending_count = 0
            for pending in create_batch.to_create:
                if (pending.get('property_id') == property_id and
                    pending.get('checkin') == checkin_date and
                    pending.get('checkout') == checkout_date and
                    pending.get('entry_type') == entry_type):
                    # Create a fake record structure
                    fake_record = {
                        'id': f'pending_collector_{pending_count}',
                        'fields': pending['data']
                    }
                    all_property_records.append(fake_record)
                    exact_duplicates.append(fake_record)
                    pending_count += 1
            if pending_count > 0:
                logging.info(f"🔍 DUPLICATE CHECK: Found {pending_count} matching records in InMemoryCollector")
...
def sync_ics_event(event, existing_records, url_to_prop, table, create_batch, update_batch, session_tracker=None):
    ...
    FIXED: Checks session tracker FIRST to prevent duplicates from dynamic UID systems like Lodgify.
    """
    ...
    # Use composite UID for lookups FIRST to check if this is a modification
    all_records = existing_records.get((composite_uid, feed_url), [])
    
    # CRITICAL FIX: Also check for original UID in case record was created before composite UIDs
    # This handles backward compatibility with existing records
    actual_uid_in_airtable = composite_uid  # Track what UID is actually stored in Airtable
    if not all_records and composite_uid != original_uid:
        all_records = existing_records.get((original_uid, feed_url), [])
        if all_records:
            logging.info(f"Found records using original UID {original_uid} instead of composite {composite_uid}")
            actual_uid_in_airtable = original_uid  # The record uses original UID, not composite
    
    # Filter for active records (New or Modified)
    active_records = [r for r in all_records if r["fields"].get("Status") in ("New", "Modified")]
    
    # Check for duplicates with different UIDs (for systems that don't use dynamic UIDs)
    # This is a secondary check for non-Lodgify systems
    ...
    # If we have active records with this UID, process updates/unchanged
    if active_records:
    ...
    # No records with this UID exist - check if it's a duplicate with a different UID
    ...
    # CRITICAL: Check session tracker before creating new record
    # This prevents duplicates when ICS feeds use dynamic UIDs (like Lodgify)
    ...
    # Not a duplicate and no existing records - create new
    new_fields = {
...
def process_ics_feed(url, events, existing_records, url_to_prop, table, create_batch, update_batch, session_tracker=None):
    ...
    for event in events:
        ...
        # CRITICAL FIX: Track BOTH UIDs to handle the transition from original to composite
        # When we update a record, its UID changes from "43773" to "43773_recPropertyID"
        # We must track both to prevent false removals
        processed_uid_url_pairs.add((uid, url))  # Always track original UID
        if composite_uid != uid:
            processed_uid_url_pairs.add((composite_uid, url))  # Also track composite if different
            logging.debug(f"Tracking both UIDs for removal detection: original={uid}, composite={composite_uid}")
    ...
    # IMPORTANT REMOVAL LOGIC:
    # - Only remove records where checkout date is IN THE FUTURE (cancelled reservations)
    # - If checkout is in the past, it already happened - don't remove
    # - This prevents marking active reservations as removed
    now_iso = datetime.now(arizona_tz).isoformat(sep=" ", timespec="seconds")
    # Use Arizona timezone for today's date to match reservation data
    today_iso = datetime.now(arizona_tz).date().isoformat()
    removed_count = 0
    ...
    for i, (uid, feed_url) in enumerate(missing_keys):
        ...
        for rec in active_records:
            ...
            # IMPORTANT: Only remove records where checkout is in the FUTURE (cancelled)
            # If checkout is in the past OR TODAY, it already happened/is happening - don't remove
            checkout_date = fields.get("Check-out Date", "")
            if checkout_date <= today_iso:
                logging.info(f"⚠️ SKIP REMOVAL: Record {record_id} has checkout {checkout_date} <= today {today_iso} - already happened/happening")
                continue
            ...
            if property_ids:
                ...
                # IMPORTANT: The property_id for this feed comes from url_to_prop mapping
                feed_property_id = url_to_prop.get(url)
                found_in_current_feed = False
                
                # Only check if this feed is for the same property as the record being removed
                if feed_property_id == record_property_id:
                    for feed_event in events:
                        if (feed_event.get('dtstart') == record_checkin and
                            ...
                            logging.info(f"   Property: {record_property_id}, Dates: {record_checkin} to {record_checkout}")
                ...
                try:
                    ...
                    logging.info(f"🔍 LODGIFY DUPLICATE CHECK: Checking for active duplicates of {uid}")
                    logging.info(f"   Property: {record_property_id}, Dates: {record_checkin} to {record_checkout}, Type: {record_entry_type}")
                    logging.info(f"   Formula: {duplicate_formula}")
                    
                    other_active_records = table.all(formula=duplicate_formula, max_records=1)
                    if other_active_records:
                        found_uid = other_active_records[0]['fields'].get('Reservation UID')
                        logging.info(f"✅ LODGIFY DUPLICATE FOUND: Skipping removal of {uid}")
                        logging.info(f"   Found active record with UID: {found_uid}")
                        logging.info(f"   This prevents the add/remove cycle!")
                        continue
                    else:
                        logging.info(f"❌ NO DUPLICATE FOUND: Will mark {uid} as removed")
                        logging.info(f"   This might cause add/remove cycle if hybrid detection updated the UID")
                except Exception as e:
                    logging.error(f"Error checking for duplicate records: {e}")
                    logging.error(f"   UID: {uid}, Property: {record_property_id}")
...
def generate_report(overall_stats, id_to_name):
    ...
    for feed_url, stats in overall_stats.items():
        ...
        # Format with type breakdown if available
        new_count = stats.get('New', 0)
        if new_count > 0 and (stats.get('New_Reservation', 0) > 0 or stats.get('New_Block', 0) > 0):
            new_text = f"{new_count} ({stats.get('New_Reservation', 0)} res, {stats.get('New_Block', 0)} block)"
        else:
            new_text = f"{new_count} reservations"
            
        modified_count = stats.get('Modified', 0)
        if modified_count > 0 and (stats.get('Modified_Reservation', 0) > 0 or stats.get('Modified_Block', 0) > 0):
            modified_text = f"{modified_count} ({stats.get('Modified_Reservation', 0)} res, {stats.get('Modified_Block', 0)} block)"
        else:
            modified_text = f"{modified_count} reservations"
            
        removed_count = stats.get('Removed', 0)
        if removed_count > 0 and (stats.get('Removed_Reservation', 0) > 0 or stats.get('Removed_Block', 0) > 0):
            removed_text = f"{removed_count} ({stats.get('Removed_Reservation', 0)} res, {stats.get('Removed_Block', 0)} block)"
        else:
            removed_text = f"{removed_count} reservations"
        
        logging.info(f" * New: {new_text}")
        logging.info(f" * Modified: {modified_text}")
        logging.info(f" * Unchanged: {stats.get('Unchanged', 0)} reservations")
        logging.info(f" * Removed: {removed_text}")
    
    # Calculate type breakdown for summary
    total_new_res = sum(stats.get("New_Reservation", 0) for stats in overall_stats.values())
    total_new_block = sum(stats.get("New_Block", 0) for stats in overall_stats.values())
    total_mod_res = sum(stats.get("Modified_Reservation", 0) for stats in overall_stats.values())
    total_mod_block = sum(stats.get("Modified_Block", 0) for stats in overall_stats.values())
    total_rem_res = sum(stats.get("Removed_Reservation", 0) for stats in overall_stats.values())
    total_rem_block = sum(stats.get("Removed_Block", 0) for stats in overall_stats.values())
    
    # Print summary (will go to both console and log file)
    logging.info("------------  Summary  ------------")
    logging.info(f"Total unchanged: {total_unchanged}")
    logging.info(f"Total new: {total_new} (Reservations: {total_new_res}, Blocks: {total_new_block})")
    logging.info(f"Total modified: {total_modified} (Reservations: {total_mod_res}, Blocks: {total_mod_block})")
    logging.info(f"Total removed: {total_removed} (Reservations: {total_rem_res}, Blocks: {total_rem_block})")
    logging.info("------------  End Summary  ------------")

# ---------------------------------------------------------------------------
# Memory Collection Processing Function
# ---------------------------------------------------------------------------
@ultra_timing_debug("ICS")
def process_with_memory_collection(reservations_table, feed_results, existing_records, url_to_prop_map, id_to_name, session_tracker):
    """Process all feeds with in-memory collection to prevent creating-then-marking-old."""
    
    log_critical_timing("🚨 ULTRA DEBUG: Starting memory collection processing")
    # Initialize collector
    collector = InMemoryCollector()
    collector.property_mapping = id_to_name  # For display purposes
    collector.existing_records = existing_records  # Pass existing records
    
    # Process each feed and collect operations
    overall_stats = {}
    # Use the session_tracker that was already initialized above, don't create a new one!
    ...
    for url, result in feed_results.items():
        ...
        # Track stats for this feed - separate by type
        stats = {
            "New": 0, "Modified": 0, "Unchanged": 0, "Duplicate_Ignored": 0,
            "New_Reservation": 0, "New_Block": 0,
            "Modified_Reservation": 0, "Modified_Block": 0,
            "Removed_Reservation": 0, "Removed_Block": 0
        }
        ...
        for event in events:
            ...
            # Session tracker for duplicates within this run
            if property_id:
                tracker_key = (property_id, event['dtstart'], event['dtend'], event['entry_type'])
                if session_tracker is not None and tracker_key in session_tracker:
                    stats['Duplicate_Ignored'] += 1
                    logging.info(f'🚫 SESSION TRACKER: Preventing duplicate within this run {original_uid} for property {property_id}')
                    continue
            ...
            # Skip if already processed (check both UID formats)
            if (original_uid, url) in collector.processed_uids or (composite_uid, url) in collector.processed_uids:
                continue
            
            # CRITICAL FIX: Track BOTH UIDs to handle the transition from original to composite
            # When we update a record, its UID changes from "43773" to "43773_recPropertyID"
            # We must track both to prevent false removals
            collector.processed_uids.add((original_uid, url))  # Always track original UID
            if composite_uid != original_uid:
                collector.processed_uids.add((composite_uid, url))  # Also track composite if different
                logging.debug(f"Tracking both UIDs for removal detection: original={original_uid}, composite={composite_uid}")
            
            # LODGIFY FIX: Track property+dates+type for ALL events
            # This prevents removal when Lodgify changes UIDs
            if property_id:
                checkin_date = extract_date_only(event['dtstart'])
                checkout_date = extract_date_only(event['dtend'])
                entry_type = event['entry_type']
                collector.processed_property_dates.add((property_id, checkin_date, checkout_date, entry_type))
                logging.debug(f"📍 Tracking property+dates for {original_uid}: {property_id}, {checkin_date} to {checkout_date}, {entry_type}")
            ...
            # This handles backward compatibility with existing records
            if not all_records and composite_uid != original_uid:
                all_records = existing_records.get((original_uid, url), [])
                if all_records:
                    logging.info(f"Found records using original UID {original_uid} instead of composite {composite_uid}")
            
            # HYBRID APPROACH: If no UID match, try property+dates+type matching
            # This handles Lodgify's dynamic UIDs
            if not all_records and property_id:
                # Search for records with same property, dates, and type
                # Note: checkin_date, checkout_date already set above
                # entry_type is from event['entry_type']
                
                # Check all existing records for this match
                for (existing_uid, existing_url), records in existing_records.items():
                    if existing_url != url:  # Must be same feed URL
                        continue
                    for record in records:
                        fields = record['fields']
                        if (fields.get('Property ID', [None])[0] == property_id and
                            ...
                            # Found a match! Use this record
                            logging.info(f"🔍 HYBRID: Found existing record by property+dates+type for UID {original_uid}")
                            logging.info(f"   Old UID: {existing_uid[0]}, New UID: {composite_uid}")
                            all_records = records
                            break
                    if all_records:
                        break
            ...
            if active_records:
                ...
                latest_active = max(active_records, key=lambda r: r["fields"].get("Last Updated", ""))
                if has_changes(event, latest_active):
                    # CRITICAL: Copy ALL fields from existing record first (except blacklisted)
                    existing_fields = latest_active["fields"]
                    update_data = {k: v for k, v in existing_fields.items() if k not in WRITE_BLACKLIST}
                    
                    # Then update with new values from the event
                    update_data.update({
                        "Reservation UID": composite_uid,  # MUST include UID!
                        "ICS URL": url,
                        "Check-in Date": event["dtstart"],
                        "Check-out Date": event["dtend"],
                        "Entry Type": event["entry_type"],
                    ...
                    if event["block_type"]:
                        update_data["Block Type"] = event["block_type"]
                    
                    if property_id:
                        update_data["Property ID"] = [property_id]
                    
                    # Mark old records for update
                    for record in all_records:
                        collector.add_update(record["id"], {"Status": "Old"})
                    
                    # Add new record creation
                    collector.add_create(composite_uid, url, update_data)
                    stats["Modified"] += 1
                    # Track by type
                    if event["entry_type"] == "Reservation":
                        stats["Modified_Reservation"] += 1
                    else:
                        stats["Modified_Block"] += 1
                else:
                    stats["Unchanged"] += 1
            else:
                # Create new record
                ...
                collector.add_create(composite_uid, url, new_fields)
                stats["New"] += 1
                # Track by type
                if event["entry_type"] == "Reservation":
                    stats["New_Reservation"] += 1
                else:
                    stats["New_Block"] += 1
        
        # Find removed reservations
        feed_keys = [(uid, feed_url) for uid, feed_url in existing_records.keys() if feed_url == url]
        missing_keys = [pair for pair in feed_keys if pair not in collector.processed_uids]
        ...
        for uid, feed_url in missing_keys:
            ...
            for rec in active_records:
                ...
                # If checkout is in the past OR TODAY, it already happened/is happening - don't remove
                checkout_date = fields.get("Check-out Date", "")
                if checkout_date <= date.today().isoformat():
                    logging.debug(f"⚠️ SKIP REMOVAL: Record has checkout {checkout_date} <= today - already happened/happening")
                    continue
                ...
                # LODGIFY PROTECTION: Check if this property+dates+type was processed in current run
                property_id = fields.get('Property ID', [None])[0] if fields.get('Property ID') else None
                checkin = fields.get('Check-in Date', '')
                checkout = fields.get('Check-out Date', '')
                entry_type = fields.get('Entry Type', '')
                
                if property_id and (property_id, checkin, checkout, entry_type) in collector.processed_property_dates:
                    logging.info(f"🛡️ LODGIFY PROTECTION: Skipping removal of {uid} - same property+dates+type was processed in current run")
                    logging.info(f"   Property: {property_id}, Check-in: {checkin}, Check-out: {checkout}, Type: {entry_type}")
                    continue
                
                # Mark for removal
                collector.add_remove(rec)
        
        # Store stats
        overall_stats[url] = stats
    
    # Cross-reference operations to detect UID changes
    uid_changes = collector.cross_reference_operations()
    
    # Execute operations
    logging.info("\n📤 EXECUTING OPERATIONS...")
    log_critical_timing("🚨 ULTRA DEBUG: Starting operations execution")
    
    # Create new reservations
    if collector.to_create:
        # CRITICAL DEBUGGING: Log exact time when starting create operations
        import datetime as dt
        log_critical_timing(f"🚨 ULTRA DEBUG: About to create {len(collector.to_create)} new reservations")
        logging.info(f"\n🔍 CRITICAL: Starting creation of {len(collector.to_create)} new reservations at {dt.datetime.now().strftime('%H:%M:%S.%f')[:12]}")
        
        log_critical_timing("🚨 ULTRA DEBUG: Creating BatchCollector for record creation")
        create_batch = BatchCollector(reservations_table, batch_size=10, op="create")
        
        for i, create_op in enumerate(collector.to_create):
            uid = create_op['data'].get('Reservation UID', 'unknown')
            prop_id = create_op['data'].get('Property ID', ['unknown'])[0] if create_op['data'].get('Property ID') else 'unknown'
            log_critical_timing(f"🚨 ULTRA DEBUG: Adding record {i+1} to batch: UID={uid}, Property={prop_id}")
            create_batch.add({"fields": create_op['data']})
        
        log_critical_timing("🚨 ULTRA DEBUG: Calling BatchCollector.done() to execute creates")
        create_batch.done()
        log_critical_timing("🚨 ULTRA DEBUG: BatchCollector.done() completed")
        logging.info(f"🔍 CRITICAL: Finished creating reservations at {dt.datetime.now().strftime('%H:%M:%S.%f')[:12]}")
    
    # Update existing reservations
    if collector.to_update:
        logging.info(f"\n📝 Updating {len(collector.to_update)} reservations...")
        update_batch = BatchCollector(reservations_table, batch_size=10, op="update")
        for update_op in collector.to_update:
            update_batch.add({
                "id": update_op['record_id'],
                "fields": update_op['data']
            })
        update_batch.done()
    ...
    if collector.to_remove:
        ...
        # Create new "Removed" records
        now_iso = datetime.now(arizona_tz).isoformat(sep=" ", timespec="seconds")
        for remove_op in collector.to_remove:
            records = [remove_op['record']]
            mark_all_as_old_and_clone(reservations_table, records, {}, now_iso, "Removed")
    
    # Update overall stats - count removals by type
    for url in overall_stats:
        stats = overall_stats[url]
        stats["Removed"] = sum(1 for r in collector.to_remove if r['feed_url'] == url)
        # Count removals by type
        for r in collector.to_remove:
            if r['feed_url'] == url:
                entry_type = r['record']['fields'].get('Entry Type', 'Block')
                if entry_type == 'Reservation':
                    stats["Removed_Reservation"] += 1
                else:
                    stats["Removed_Block"] += 1
    
    # Summary
    logging.info("\n" + "=" * 80)
    logging.info("✅ ICS SYNC COMPLETE (WITH MEMORY COLLECTION)")
    logging.info("=" * 80)
    logging.info(f"New reservations: {len(collector.to_create)}")
    logging.info(f"Updated reservations: {len(collector.to_update)}")
    logging.info(f"Removed reservations: {len(collector.to_remove)}")
    logging.info(f"UID changes detected: {len(uid_changes)}")
    
    if uid_changes:
        logging.info("\n🔄 UID CHANGES PREVENTED:")
        for change in uid_changes[:5]:  # Show first 5
            logging.info(f"   {change['property']} - {change['dates']}")
            logging.info(f"     Old: {change['old_uid'][:30]}...")
            logging.info(f"     New: {change['new_uid'][:30]}...")
...
async def main_async():
    log_critical_timing("🚨 ULTRA DEBUG: ICS main_async() starting")
    ...
    try:
        ...
        # 7. Process all feeds using memory collection
        overall_stats = process_with_memory_collection(
            reservations_table=reservations_table,
            feed_results=feed_results,
            existing_records=existing_records,
            url_to_prop_map=url_to_prop_map,
        ...
        # Log final completion message with breakdown by type
        ...
        new_reservations = sum(stats.get("New_Reservation", 0) for stats in overall_stats.values())
        new_blocks = sum(stats.get("New_Block", 0) for stats in overall_stats.values())
        modified_reservations = sum(stats.get("Modified_Reservation", 0) for stats in overall_stats.values())
        modified_blocks = sum(stats.get("Modified_Block", 0) for stats in overall_stats.values())
        removed_reservations = sum(stats.get("Removed_Reservation", 0) for stats in overall_stats.values())
        removed_blocks = sum(stats.get("Removed_Block", 0) for stats in overall_stats.values())
        
        # Build detailed summary
        new_detail = f"{total_new} ({new_reservations} res, {new_blocks} block)"
        modified_detail = f"{total_modified} ({modified_reservations} res, {modified_blocks} block)"
        removed_detail = f"{total_removed} ({removed_reservations} res, {removed_blocks} block)"
        
        logging.info(f"ICS Sync complete — new {new_detail} — modified {modified_detail} — "
                    f"unchanged {total_unchanged} — removed {removed_detail}")
        ...
        # Include breakdown in summary
        print(f"ICS_SUMMARY: Feeds={total_feeds}, Success={success_feeds}, "
              f"Failed={failed_feeds}, New={total_new}, Modified={total_modified}, "
              f"Unchanged={total_unchanged}, Removed={total_removed}, Errors={failed_feeds}, "
              f"NewDetail={new_detail}, ModifiedDetail={modified_detail}, "
              f"RemovedDetail={removed_detail}")
        
        # Run duplicate detection tests after successful processing
        try:
            logging.info("\n" + "="*60)
            logging.info("Running duplicate detection tests...")
            test_passed = run_duplicate_detection_tests("ICS", Config.environment)
            if not test_passed:
...
@ultra_timing_debug("ICS")
def main():
    log_critical_timing("🚨 ULTRA DEBUG: ICS main() starting")
    # Run the async main
    try:
