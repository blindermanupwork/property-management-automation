/**********************************************************************
 *  Airbnb Turn-over  ←→  Housecall Pro one-click automator (PRODUCTION)
...
/* ─ 1) pick reservation ────────────────────────────────────────── */
output.markdown("## 🔎 Select a reservation to sync (PRODUCTION)");
...
if (!jobId) {
  ...
  if (appointmentId) {
    output.text(`➕ Created job ${jobId} with appointment ${appointmentId} (PRODUCTION)`);
  } else {
    output.text(`➕ Created job ${jobId} (no appointment ID found) (PRODUCTION)`);
  }
}
...
output.text(`🗸 ${reservationUID}: ${syncStatus} (PRODUCTION)`);