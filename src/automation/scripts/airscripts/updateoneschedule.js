...
if (!HCP_TOKEN) {
  output.text("❌ Error: Could not find HCP token in Secrets table (KeyName: 'hcpdev')");
  return;
}

// Get the selected record from the Reservations table
let reservationsTable = base.getTable("Reservations");
output.text("Running sync status check...");
let currentRecord = await input.recordAsync(
  "Select a Reservation to Check",
  reservationsTable
);
if (!currentRecord) {
...
output.text("Checking record ID: " + currentRecord.id);
...
try {
  ...
  if (!jobDetails.schedule.scheduled_start) {
    ...
    output.text("✅ Schedule created successfully!");
  ...
  if (needsUpdate) {
    output.text(`${syncStatus} detected. Updating HCP schedule...`);
    ...
    output.text("✅ Schedule updated in HCP.");
  ...
  output.text(`🔄 Attempting to update Sync Status to: "${syncStatus}"`);
  try {
    await reservationsTable.updateRecordAsync(currentRecord.id, {
    ...
    output.text(`✅ Successfully updated Sync Status to: "${syncStatus}"`);
  } catch (statusError) {
    output.text(`⚠️ Could not set Sync Status to '${syncStatus}' - Error: ${statusError.message}`);
  ...
  output.text(`✅ Sync complete: ${syncStatus}`);
  ...
  try {
    await reservationsTable.updateRecordAsync(currentRecord.id, {
      "Sync Details": "Error: " + err.message,
      "Sync Date and Time": new Date().toISOString()
    });
  } catch (updateError) {
    output.text("⚠️ Could not update error details: " + updateError.message);
  }
}

output.text("Sync check finished.");