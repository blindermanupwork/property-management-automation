...
# ── Configuration (adapted for Linux) ────────────────────────────────────────
DEFAULT_DOWNLOAD_FOLDER = os.path.expanduser("~/automation/CSV_process")

# Default parameters
SENDER_OPTIONS = ["scottsdaleitrip@itrip.net", "scottsdaleinfo@itrip.net"]  
SUBJECT = "iTrip Checkouts Report"

SCOPES = ['https://www.googleapis.com/auth/gmail.readonly']
TOKEN_PATH = os.path.expanduser("~/automation/scripts/gmail/token.pickle")
CREDS_PATH = os.path.expanduser("~/automation/scripts/gmail/credentials.json")
...
if __name__ == "__main__":
    ...
    if not success and not args.list_only:
        ...
        logger.info("  - List all recent emails: python3 gmail_downloader_linux.py --list-only --all-emails")
        logger.info("  - Ignore sender (search by subject only): python3 gmail_downloader_linux.py --ignore-sender")
        logger.info("  - Force re-download of emails: python3 gmail_downloader_linux.py --force")
        logger.info("  - Use a Gmail label instead: python3 gmail_downloader_linux.py --use-label \"iTrip\"")
        logger.info("  - Check emails from yesterday: python3 gmail_downloader_linux.py --days 1")
        logger.info("  - Show debug info: python3 gmail_downloader_linux.py --debug")
        logger.info("  - Specify download location: python3 gmail_downloader_linux.py --output-dir \"PATH\"")
