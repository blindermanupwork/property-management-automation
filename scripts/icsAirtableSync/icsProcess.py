...
future_start_threshold = None
future_end_threshold = None
ignore_past = False
ignore_future_start = False
ignore_future_end = False

LOG_FILE = "ics_sync.log"
...
def get_active_ics_urls_from_airtable(feeds_table):
    ...
    try:
        # Assuming 'Feed Status' field exists and has 'Active' option
        formula = "{Feed Status} = 'Active'"
        # Fetch URL and the linked Property record ID field
        records = feeds_table.all(fields=["ICS URL", property_link_field_name], formula=formula)
        ...
        for record in records:
            ...
            if url and prop_links and isinstance(prop_links, list) and len(prop_links) > 0:
                ...
                logging.warning(f"Active ICS URL '{url}' is missing its Property link ('{property_link_field_name}' field) in the '{feeds_table.name}' table. Skipping URL.")
...
def parse_ics(ics_text: str, feed_url: str):
    ...
    try:
        ...
        for component in cal.walk():
            ...
            # now it's safe to compare dtend < today or dtstart > future threshold
            if lookback_threshold and dtstart < lookback_threshold:
                skipped_past += 1
                continue
            if ignore_past and dtend < today:
                skipped_past += 1
                continue
...
async def main_async():
    ...
    try:
        ...
        required_vars = [AIRTABLE_API_KEY, AIRTABLE_BASE_ID, AIRTABLE_TABLE_NAME, 
                        PROPERTIES_TABLE_NAME, ICS_FEEDS_TABLE_NAME]
        if not all(required_vars):
            missing = [name for name, val in zip(['AIRTABLE_API_KEY', 'AIRTABLE_BASE_ID', 
                                                'AIRTABLE_TABLE_NAME', 'PROPERTIES_TABLE_NAME', 
                                                'ICS_FEEDS_TABLE_NAME'], required_vars) if not val]
        ...
        today = date.today()
        ignore_past = getenv_bool("IGNORE_EVENTS_ENDING_BEFORE_TODAY", False)        
        # Lookback threshold
        months_before = getenv_int("FETCH_RESERVATIONS_MONTHS_BEFORE", 0)
        if months_before > 0:
            lookback_threshold = today - relativedelta(months=months_before)
            logging.info(f"Including events starting after {lookback_threshold.isoformat()} (back {months_before} months)")
        else:
            lookback_threshold = None
            
        # Future thresholds
        try:
            start_months = getenv_int('IGNORE_BLOCKS_MONTHS_AWAY', None)
            if start_months is not None:
                future_start_threshold = today + relativedelta(months=start_months)
                ignore_future_start = True
                logging.info(f"Ignoring events starting after {future_start_threshold.isoformat()} (>{start_months} months away)")
            else:
                future_start_threshold = None
                ignore_future_start = False
                logging.warning("Could not load IGNORE_BLOCKS_MONTHS_AWAY; not ignoring distant starts")
        except Exception:
            future_start_threshold = None
            ignore_future_start = False
            logging.warning("Could not load IGNORE_BLOCKS_MONTHS_AWAY; not ignoring distant starts")

        # end‑future threshold
        end_months = getenv_int("IGNORE_EVENTS_ENDING_MONTHS_AWAY", None)
        if end_months is not None:
            future_end_threshold = today + relativedelta(months=end_months)
            ignore_future_end = True
            logging.info(f"Ignoring events ending after {future_end_threshold.isoformat()} (>{end_months} months away)")
        else:
            future_end_threshold = None
            ignore_future_end = False
            logging.info("No IGNORE_EVENTS_ENDING_MONTHS_AWAY set; not ignoring distant ends")
        ...
        id_to_name = {}
        try:
            prop_records = properties_table.all(fields=[os.getenv('PROPERTIES_NAME_FIELD', 'Property Name')])
            for rec in prop_records:
                rec_id = rec["id"]
                name = rec["fields"].get(os.getenv('PROPERTIES_NAME_FIELD', 'Property Name'), "")
                if name:
